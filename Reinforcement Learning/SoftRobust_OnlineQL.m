%% Aggressive, Soft-Robust and Robust Policy for MDP via online Q-learning
...
%% Online Q-learning for different modes
...
recordSAR=zeros(numModes,numEpisodes,3);
...
for mode=modes
    ...
    for episode=1:numEpisodes
        while true
            ...
            S_next=discreteSamples(reshape(P_nom(S,A,:),1,[]),1);
            R=R_nom(S,A,S_next);
            recordSAR(i,episode,:)=[S,A,R];

            % Q-learning update
            ISF=P(S,A,S_next)/P_nom(S,A,S_next);
            Q(S,A)=Q(S,A)+ ...
                  stepSizeQ*(ISF*R+discountFactor*max(Q(S_next,:))-Q(S,A));
            S=S_next;
...
%% Online Q-learning results
f=figure;
hold on
z=((1:length(recordSAR))');
plot(cumsum(reshape(recordSAR(1,:,3),[],1))./z,'-r')
plot(cumsum(reshape(recordSAR(2,:,3),[],1))./z,'-b')
plot(cumsum(reshape(recordSAR(3,:,3),[],1))./z,'-g')
legend(modes)
ylabel("Average Reward")
xlabel("Decision Epochs")
title("Online Q-learning")
hold off
exportgraphics(f,"Online-Q-learning.png","Resolution",300);
